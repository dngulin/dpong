...
namespace DPong.Level.AI {
  public class AiInputSource {
    public Keys GetLeft(LevelState state) => GetKeys(state.Paddles[0].Position, state.Ball.Position);

    public Keys GetRight(LevelState state) => GetKeys(state.Paddles[1].Position, state.Ball.Position);
